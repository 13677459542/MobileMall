...
<template>
  <div class="order-list-item" v-if="item.order_id">
    ...
    <div class="actions">
      <div v-if="item.order_status === 1 || item.order_status === 2 || item.order_status === 3">
        <span v-if="item.order_status === 1">立刻付款</span>
        <span v-if="item.order_status === 1">申请取消</span>
        <span v-if="item.order_status === 2 || item.order_status === 3">确认收货</span>
      </div>
