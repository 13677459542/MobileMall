...
const logoutApi = () => {
  showConfirmDialog({
    ...
    .then(() => {
      // 确认退出
      console.log('触发了退出登录')
      userStore.logout(cartStore)
      cartStore.setCartList([])
      console.log('cartList清理结束')
      router.push({
        path: '/login',
      })
      console.log('用户退出')
...
<template>
  <div class="user">
    ...
    <div class="order-navbar">
      <div class="order-navbar-item" @click="$router.push('/order/MyOrder?dataType=0')">
        <van-icon name="balance-list-o" />
        <span>全部订单</span>
      </div>
      <div class="order-navbar-item" @click="$router.push('/order/MyOrder?dataType=1')">
        <van-icon name="clock-o" />
        <span>待支付</span>
      </div>
      <div class="order-navbar-item" @click="$router.push('/order/MyOrder?dataType=2')">
        <van-icon name="logistics" />
        <span>待发货</span>
      </div>
      <div class="order-navbar-item" @click="$router.push('/order/MyOrder?dataType=3')">
    ...
    <div class="logout-btn">
      <button @click="logoutApi">退出登录</button>
    </div>
  </div>
</template>
